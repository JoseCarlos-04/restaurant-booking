...
export class BookingService {
  ...
  deleteBook(id: number): void{
    let index: number = this.bookings.findIndex((book: Booking) => book.id === id);
    this.bookings.splice(index, 1);
  }

  addBook(book: Booking) {
    const index = this.bookings.findIndex(bookings => book.id === bookings.id);

    console.log(index);

    if (index <= -1) {
      book.id = Math.trunc(Math.random() * 10000) + 1;

      this.bookings.push(book);

      console.log(book);
    } else {
      this.bookings[index] = book;
    }
  }

  getBookById(id: number): Booking|null {
